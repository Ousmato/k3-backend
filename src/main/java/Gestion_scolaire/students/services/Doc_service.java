...
import Gestion_scolaire.students.dtos.DocDTO;
import Gestion_scolaire.students.dtos.ProgramSoutenanceDto;
import Gestion_scolaire.students.dtos.SoutenanceDTO;
...
import Gestion_scolaire.students.repositories.Doc_repositorie;
import Gestion_scolaire.students.repositories.Inscription_repositorie;
import Gestion_scolaire.students.repositories.Soutenance_repositorie;
import Gestion_scolaire.students.repositories.StudentDoc_repositorie;
...
//   -------------------------------
    public Object addSoutenanceNote(long idDoc, double note){
        ...
        if(doc != null){
            ...
            return DTO_response_string.fromMessage("Note ajouter avec succès");
        }
        return null;
    }
    ...
    public Object annulerDepot(long idIncrit){
        ...
        if(studentDocExist != null){
            Soutenance soutenanceExist = soutenance_repositorie.getByIdDocIdDocumentIdAndIdDocIdDocumentDeleted(studentDocExist.getIdDocument().getId(), true);
            if(soutenanceExist != null){
                throw new NoteFundException("Impossible d'annuler un depot  deja programmer pour la soutenance");
            }
            studentDocExist.getIdDocument().setDeleted(true);
            doc_repositorie.save(studentDocExist.getIdDocument());
            return DTO_response_string.fromMessage("Dépot annuler avec succès");
        }
